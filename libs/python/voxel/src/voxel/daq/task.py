...
class AcqTiming(BaseModel):
    ...
    def validate_duration_and_sample_rate(cls, t):
        if t.sample_rate < 2 * t.frequency:
            raise ValueError(f"sample_rate ({t.sample_rate} Hz) must be ≥ 2× clock_freq ({t.frequency} Hz)")
        return t
...
class AcqTaskConfig(BaseModel):
    ...
    def insert_missing_windows(cls, m):
...
class AcquisitionTask:
    ...
    def _initialize_channels(self, waveforms: dict[str, "Waveform"]) -> dict[str, WaveGenChannel]:
        ...
        for name, waveform in waveforms.items():
            if name not in self._pins:
                raise ValueError(f"Pin {name} not assigned for waveform: {waveform}")
    ...
    def _configure_timing(self) -> None:
        if not self._inst:
            raise RuntimeError("Task instance is not initialized.")
    ...
    def plot(self, clock_cycles: int = 1):
