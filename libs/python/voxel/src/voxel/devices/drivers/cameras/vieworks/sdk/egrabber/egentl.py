...
class EGenTL:
    ...
    def image_get_bytes_per_pixel(self, format):
        ...
        bps = ct.c_uint()
        cE.Eur_EGenTL_imageGetBytesPerPixel__from__const_char_p(self._box, utils.to_cstr(format), ct.byref(bps))
        return bps.value
    ...
    def image_get_bits_per_pixel(self, format):
        ...
        if type(format) == int:
            format = self.image_get_pixel_format(format)
        bps = ct.c_uint()
        cE.Eur_EGenTL_imageGetBitsPerPixel__from__const_char_p(self._box, utils.to_cstr(format), ct.byref(bps))
        return bps.value
...
class ImageConvertInput(ct.Structure):
    ...
    def __init__(self, width, height, pixels, format, buffer_size=None, line_pitch=None):
        ...
        super(ImageConvertInput, self).__init__(
            width, height, pixels, utils.to_cstr(format), buffer_size, line_pitch, None, None, 1, 0, 0, 0
        )
...
class ImageConvertOutput(ct.Structure):
    ...
    def __init__(self, width, height, pixels, format, config, operation, quality, size=None):
        ...
        super(ImageConvertOutput, self).__init__(
            width, height, ct.addressof(pixels), utils.to_cstr(format), config, operation, 1, quality, size
        )
...
class ImageSaveToDiskParams(ct.Structure):
    ...
    _fields_ = [("_config", ct.c_int), ("quality", ct.c_int), ("reserved", ct.c_int * 4)]
