...
import pytest
from rigup.device import CommandRequest, DeviceInterface, PropResults, Results
...
class TestDaemonAuthority:
    async def test_claim_and_release(self, daemon_pair):
        ...
        resp = await call(
            client, Action.RELEASE, ReleaseRequest(orchestrator_id="rig-1"), ReleaseResponse, timeout_s=5.0
        )
...
class TestDaemonDeviceLifecycle:
    ...
    async def test_close_device(self, daemon_pair):
        ...
        await call(
            client,
            Action.BUILD_DEVICES,
            BuildDevicesRequest(devices={"dev": DeviceConfig(target=MOCK_TARGET, init={})}),
            BuildDevicesResponse,
            timeout_s=10.0,
        )
    ...
    async def test_build_clears_previous(self, daemon_pair):
        ...
        await call(
            client,
            Action.BUILD_DEVICES,
            BuildDevicesRequest(devices={"old": DeviceConfig(target=MOCK_TARGET, init={})}),
            BuildDevicesResponse,
            timeout_s=10.0,
        )
        ...
        await call(
            client,
            Action.BUILD_DEVICES,
            BuildDevicesRequest(devices={"new": DeviceConfig(target=MOCK_TARGET, init={})}),
            BuildDevicesResponse,
            timeout_s=10.0,
        )
    ...
    async def test_build_with_bad_target(self, daemon_pair):
        ...
        resp = await call(
            client,
            Action.BUILD_DEVICES,
            BuildDevicesRequest(devices={"bad": DeviceConfig(target="nonexistent.Mod", init={})}),
            BuildDevicesResponse,
            timeout_s=10.0,
        )
...
class TestDaemonDeviceRPC:
    ...
    async def built_daemon(self, daemon_pair):
        ...
        await call(
            client,
            Action.BUILD_DEVICES,
            BuildDevicesRequest(devices={"dev": DeviceConfig(target=MOCK_TARGET, init={"initial_value": 5.0})}),
            BuildDevicesResponse,
            timeout_s=10.0,
        )
