...
### New `rigup/rig.py` — Rig class (no ClusterManager)
- **Where:** `src/rigup/rig.py`.
...
- **Key divergence from rigup:**
    - **No `ClusterManager`.** rigup has `Rig` → `ClusterManager` → nodes.
      rigup's `Rig` directly owns `Node` instances. The coordinator class
    ...
    - **No `node_cls()` classmethod.** rigup's extension model was
      `Rig.node_cls()` → `VoxelNode` → custom controllers/handles. rigup
...
- **Where:** `pyproject.toml` → `rigup.node._runner:standalone_main`.
...
### New `rigup/node/` subpackage — full node layer
- **Where:** `src/rigup/node/{__init__.py,_base.py,_local.py,_transport.py,
...
- **Key divergence from rigup:**
    ...
    - rigup's `RigNode` was monolithic (server-side service); rigup splits
...
### Rewrote `rigup/protocol.py` as the typed action vocabulary + dispatch layer
- **Where:** `src/rigup/protocol.py` (replaces the old lift from rigup that
...
- **What:**
    ...
    - Pydantic payload models for every action (`ClaimRequest`,
      ...
      RPC responses are re-exported from `rigup.device` (`Results`,
...
### New `rigup/transport/` package — per-node, bidirectional, wire-agnostic
- **Where:** `src/rigup/transport/{_base.py,_zmq.py,__init__.py}`.
  ...
  `transport/__init__.py` so callers write `from rigup.transport import
  ZMQTransportClient` and never see the underscore.
  Old per-device code lives in `src/rigup/transport_old/` for reference only.
...
- **Where:** `src/rigup/build.py` (was `src/rigup/device/build.py`).
...
### New `rigup/config.py` with `RigConfig` / `NodeConfig`
- **Where:** `src/rigup/config.py`.
...
- **Why:** Needed for `Signal` on `Adapter.props_changed`. Accepted as a direct
  runtime dependency for now; revisit if rigup ever needs to stand fully alone.
