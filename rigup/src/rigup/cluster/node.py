...
def main(service_cls: type[RigNode] = RigNode):
    ...
    Usage:
        python -m rigup.node <node_id> [controller_host] [control_port] [log_port] [start_port]

    Examples:
        python -m rigup.node camera_1
        python -m rigup.node camera_1 localhost 9000 9001

    Args:
        service_cls: RigNode class to instantiate (default: RigNode)
    """
    if len(sys.argv) < 2:
        print("Usage: python -m rigup.node <node_id> [controller_host] [control_port] [log_port] [start_port]")
        print("Example: python -m rigup.node camera_1 localhost 9000 9001")
        sys.exit(1)
