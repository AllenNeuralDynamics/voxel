...
back with `ZarrLoader` and checks the per-frame content.
"""
...
from ome_zarr_writer import OMEZarrWriter, PyramidRingBuffer, ScaleLevel, UIVec3D, UVec3D, WriterConfig
from ome_zarr_writer.viewer.loader import ZarrLoader


def main(directory: Path | S3Path | None = None, filename: str = "stack") -> None:
    z, y, x = 256, 4096, 4096
    directory = Path(tempfile.mkdtemp()) if directory is None else directory
    target = directory / f"{filename}.ome.zarr"
    ...
    writer = OMEZarrWriter(cfg, ring_buffer=PyramidRingBuffer.PROCESS, slots=3)
    for i in range(z):
        writer.add_frame(np.full((y, x), i + 1, dtype=np.uint16))  # frame i holds value i+1
    writer.close()

    l0 = ZarrLoader(target / "0").get_3d(z0=0, z1=z)
    assert l0.shape == (z, y, x), l0.shape
    assert int(l0[0].max()) == 1 and int(l0[z - 1].max()) == z
    print(f"wrote + verified {z} frames at {target}")
