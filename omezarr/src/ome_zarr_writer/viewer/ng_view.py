...
from .zarr_server import HTTPDZarrServer
...
class NGLayerConfig:
    def __init__(
        ...
        self._s3_sources: list[S3Path] = []
        for source in sources:
            if isinstance(source, S3Path):
    ...
    @property
    def s3_sources(self) -> list[S3Path]:
