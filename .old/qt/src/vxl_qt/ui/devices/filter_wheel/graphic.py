"""Animated SVG filter wheel graphic widget."""
...
class WheelGraphic(QWidget):
    """Animated filter wheel graphic with slot selection.

    A pure UI widget that renders an SVG-based filter wheel with:
    - Smooth rotation animation using Qt easing curves
    - Click-to-select slots
    - Hover effects with tooltips
    - Theme-aware colors

    This widget is stateless with respect to hardware - it only manages
    the visual representation. Use FilterWheelControl for device integration.

    Signals:
        selected_changed: Emitted when user selects a slot (emits slot index)
        highlighted_changed: Emitted when highlighted slot changes during animation
    """
    ...
    def selected_slot(self) -> int | None:
        """Get the currently active slot index, returns None if none."""
    ...
    def selected_slot(self, slot: int) -> None:
        """Set the currently active slot index."""
    ...
    def set_selected_slot_no_animation(self, slot: int) -> None:
        ...
        self.angle_offset = (-position_base_angle) % 360
        self.update_svg()
    ...
    def get_selected_slot_label(self) -> str | None:
        ...
        return self.get_slot_label(self.selected_slot) if self.selected_slot is not None else None
    ...
    def update_svg(self) -> None:
        ...
        wheel_outer_radius = self.orbit_radius + self.slot_size * 1.5
        wheel_inner_radius = 10  # Inner hole radius
        cutout_radius = self.slot_size + self.desired_spacing / 2
        cutout_center_x = center_x  # Cutout at 12 o'clock position
        cutout_center_y = center_y - self.orbit_radius

        # Wheel with cutout using SVG path
        wheel_path = f"""
    ...
    def _rotate_to_position(self, slot: int, clockwise: bool | None = None) -> None:
        ...
        Args:
            slot: The position to rotate to
