...
from vxl import OutputConfig, RigMode, Session
...
@info_router.patch("/metadata-schema")
async def update_metadata_schema(
    ...
    try:
        service.session.set_metadata_schema(request.schema)
        service.broadcast({}, with_status=True)
        return service.get_session_details()
...
@info_router.get("/metadata-schemas")
async def get_metadata_schemas() -> dict[str, Any]:
    """Return available metadata schemas for the session metadata schema selector."""
