...
export class Client {
  ...
  /**
   * Call a backend REST endpoint under ``/api``. Throws on non-2xx with the
   * backend's ``detail`` message when available. Returns the raw ``Response``
   * so callers can choose to parse it (or not).
   */
  ...
  async fetchSessionDetails(): Promise<SessionDetails> {
    const res = await this.request('GET', '/session/details');
    return res.json();
  }
