...
from rigur.build import BuildError
from rigur.config import NodeConfig, RigConfig
from rigur.device import DeviceHandle
from rigur.node import LocalNode, Node, RemoteNode, SubprocessNode
...
class Rig:
    ...
    def __init__(self, config: RigConfig) -> None:
        self._config = config
        self._log = logging.getLogger(f"rigur.rig.{config.name}")
