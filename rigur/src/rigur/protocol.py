...
# --- Device RPC ---
#
# RUN_COMMANDS / GET_PROPS / SET_PROPS use the rigur.device request models directly
...
async def call[Resp: BaseModel](
    ...
    *,
    ...
    Raises :class:`rigur.transport.TransportError` if the remote handler
...
# ``Results`` / ``PropResults`` / ``DeviceInterface`` are re-exported from ``rigur.device``
