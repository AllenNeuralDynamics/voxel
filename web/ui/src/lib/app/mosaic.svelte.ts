...
import type { Stage } from './axis.svelte';
import { SnapshotStore } from './snapshots.svelte';
import type { AppStatusUpdate, SessionStateUpdate, GridConfig, Tile } from './types';
...
export class MosaicManager {
  ...
  async setOffset(xOffsetUm: number, yOffsetUm: number): Promise<void> {
    try {
      await this.#client.request('PATCH', '/grid', { x_offset: xOffsetUm, y_offset: yOffsetUm });
    } catch (error) {
  ...
  async setOverlap(overlapX: number, overlapY: number): Promise<void> {
    try {
      await this.#client.request('PATCH', '/grid', { overlap_x: overlapX, overlap_y: overlapY });
    } catch (error) {
  ...
  align(edge: AlignEdge, position?: { x: number; y: number }): void {
    const stage = this.#getStage();
    if (!stage?.x || !stage?.y) return;
    const stagePos = position ?? { x: stage.x.position, y: stage.y.position };
    const { xOffsetUm, yOffsetUm } = computeAlignedOffset(
      edge,
      stagePos,
      { x: stage.x.lowerLimit, y: stage.y.lowerLimit },
  ...
  positionToCell(position: number, axis: 'x' | 'y'): number {
    ...
    const lowerLimit = axis === 'x' ? stage.x.lowerLimit : stage.y.lowerLimit;
