...
import type { AppStatusUpdate, SessionStateUpdate, Stack, StackOrder, StackStatus, PlanConfig } from './types';
...
export class StacksManager {
  ...
  async add(
    ...
    try {
      const res = await this.#client.request('POST', '/stacks', {
  ...
  async edit(
    ...
    try {
      await this.#client.request('PATCH', '/stacks', {
        edits: edits.map((e) => ({
          stack_id: e.stackId,
          ...(e.x !== undefined && { x: e.x }),
          ...(e.y !== undefined && { y: e.y }),
  ...
  async remove(stackIds: string[], undoTag?: string): Promise<void> {
    try {
      const res = await this.#client.request('DELETE', '/stacks', { stack_ids: stackIds });
  ...
  async setStackOrder(order: StackOrder): Promise<void> {
    try {
      await this.#client.request('PUT', '/stacks/order', { stack_order: order });
    } catch (error) {
  ...
  async setSortByProfile(sortByProfile: boolean): Promise<void> {
    try {
      await this.#client.request('PUT', '/stacks/order', { sort_by_profile: sortByProfile });
    } catch (error) {
  ...
  async reorderProfiles(profileIds: string[]): Promise<void> {
    try {
      await this.#client.request('PUT', '/stacks/order', { profile_order: profileIds });
    } catch (error) {
  ...
  async setDefaults(fields: { z_step?: number; default_z_start?: number; default_z_end?: number }): Promise<void> {
    try {
      await this.#client.request('PUT', '/stacks/defaults', fields);
    } catch (error) {
