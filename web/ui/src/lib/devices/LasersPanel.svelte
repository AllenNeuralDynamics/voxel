<script lang="ts">
  ...
  import { type ChannelConfig, type Instrument, LaserHandle } from '$lib/model';
  import { formatPropValue, isPropDiverged } from '$lib/prop';
  ...
  type ProfileLaser = { laser: LaserHandle; config: ChannelConfig };
  type OtherLaser = { laser: LaserHandle; config: null };
  ...
  const laserEntries = $derived<LaserEntry[]>(allLasers.map((laser) => ({ laser, config: channelFor(laser.id) })));
  ...
  /** The channel that places a laser into the active profile, or null for lasers outside it. */
  function channelFor(laserId: string): ChannelConfig | null {
    const ch = instrument.activeChannels.find((c) => c.laser.id === laserId);
    return ch ? (instrument.imaging.channels[ch.id] ?? null) : null;
  }

  /** The active profile's saved props for a device. */
  function savedProps(deviceId: string): Record<string, unknown> | undefined {
    return instrument.imaging.profiles[activeProfileId]?.props[deviceId];
  }
  ...
  function divergenceOf(entry: LaserEntry): Divergence {
    ...
    const savedPower = savedProps(entry.laser.id)?.['power_setpoint'];
...
{#snippet channelDot(laser: LaserHandle, cfg: ChannelConfig | null)}
  {#if cfg}
    <Tooltip.Provider>
              ...
              <div class="flex items-center gap-2">
                ...
                {#if profileLasers.some((pl) => divergenceOf(pl).isDirty)}
                  <Button
                    variant="ghost"
                    size="icon-xs"
                    onclick={() => toastError(instrument.applySettings())}
                  ...
                  <Button
                    ...
                    onclick={() => toastError(instrument.saveSettings())}
                  >
                    Save
                  </Button>
                {/if}
    ...
    <div class="flex h-full flex-col justify-between gap-4 border-l border-border bg-surface">
      ...
      <div class="flex max-h-48 min-h-24 flex-1 flex-col border-t border-border p-4 pt-2 pb-6">
        ...
        {#if allLasers.some((l) => l.hasHistory)}
          <svg viewBox="0 0 {GRAPH_W} 100" preserveAspectRatio="none" class="h-full w-full rounded-md bg-canvas">
              ...
              {#if l.hasHistory}
                <polyline
                  points={l.powerHistory
                    .map(
                      (s, i, a) =>
                        `${((i / Math.max(1, a.length - 1)) * GRAPH_W).toFixed(1)},${(100 - (s.v / globalMaxPower) * 100).toFixed(1)}`
                    )
                    .join(' ')}
                  fill="none"
                  stroke={l.color}
