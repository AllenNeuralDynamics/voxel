<script lang="ts">
  ...
  import type { AOSignals, ClockSource, DerivedWaveform, MicroscopeConfig, Waveform } from '$lib/app';
  import { isDerivedWaveform } from '$lib/app';
  import type { SelectOption } from '$lib/kit/Select.svelte';
  import { generateTraces, niceTicks, resolveWaveforms, voltageRange } from './generate';
  ...
  let { profiles, devices, rigCfg, acquiring, class: className }: Props = $props();

  // ──────────────────────────────── Top-level state ────────────────────────────────

  const profile = $derived(profiles.activeId ? rigCfg.profiles[profiles.activeId] : undefined);
  const canEdit = $derived(!acquiring);
