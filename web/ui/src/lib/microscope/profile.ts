...
export function discoverProfileDevices(config: MicroscopeConfig, profileId: string): ProfileDevice[] {
  ...
  // Stage axes
  if (config.stage) {
    const stageAxes = [config.stage.x, config.stage.y, config.stage.z].filter(Boolean) as string[];
...
function groupByType(config: MicroscopeConfig, channels: Record<string, ChannelConfig>): DeviceGroup[] {
  ...
  for (const deviceId of profileDevices) {
    ...
    if (deviceId in config.detection) {
      ...
      const stageDevices = [config.stage.x, config.stage.y, config.stage.z].filter(Boolean);
