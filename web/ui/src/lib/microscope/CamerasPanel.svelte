<script lang="ts">
  ...
  const profileCameraIds = $derived.by(() => {
    ...
    for (const ch of Object.values(microscope.profiles.activeChannels)) {
      if (ch.detection) ids.add(ch.detection);
    }
    return ids;
  });
  const cameras = $derived([...microscope.cameras.values()].filter((c) => profileCameraIds.has(c.id)));
  const otherCameras = $derived([...microscope.cameras.values()].filter((c) => !profileCameraIds.has(c.id)));
  ...
  watch(
    ...
    () => {
      linkedIds.clear();
      for (const c of cameras) linkedIds.add(c.id);
    }
  );
  ...
  const linkedCameras = $derived(cameras.filter((c) => linkedIds.has(c.id)));
  ...
  function delimBounds(d: NumericModel | undefined, fallback: Bounds): Bounds {
    return {
      min: d?.min ?? fallback.min,
      max: d?.max ?? fallback.max,
      step: d?.step ?? fallback.step
    };
  }
  ...
  function mergeBounds(cameras: Camera[], get: (c: Camera) => NumericModel | undefined, fallback: Bounds): Bounds {
    let min = -Infinity,
      max = Infinity,
      step = 0;
  ...
  function intersectOptions<T extends string | number>(
    cameras: Camera[],
    get: (c: Camera) => EnumeratedModel<T> | undefined
  ): T[] {
  ...
  function getConstraints(camera: Camera): CardConstraints {
    if (linkedIds.has(camera.id) && linkedCameras.length > 1) {
  ...
  function forLinked(source: Camera, fn: (c: Camera) => void) {
    if (linkedIds.has(source.id) && linkedCameras.length > 1) {
  ...
  function forLinkedAsync(source: Camera, fn: (id: string) => Promise<void> | void) {
    if (linkedIds.has(source.id) && linkedCameras.length > 1) {
      for (const cam of linkedCameras) fn(cam.id);
    } else {
      fn(source.id);
    }
  }
...
{#snippet cameraCard(camera: Camera, isOther: boolean)}
  {@const isLinked = linkedIds.has(camera.id)}
  ...
  {@const activeProfileId = microscope.profiles.activeId}
  {@const channelLabel =
    !isOther && activeProfileId
      ? (getChannelFor(microscope.config, activeProfileId, camera.id)?.config?.label ??
        getChannelFor(microscope.config, activeProfileId, camera.id)?.id)
      : undefined}
  {@const savedProps = isOther ? undefined : microscope.profiles.savedProps(camera.id)}
  {@const savedRoi = isOther ? undefined : microscope.profiles.savedRoi(camera.id)}
  ...
  {@const frameSizeMb = camera.frameSizeMb?.value}
  ...
  <div class={cn('flex flex-col rounded border border-border bg-surface px-3', isOther && 'opacity-60')}>
    ...
    <div class="flex h-ui-md items-center gap-2">
      <span class="text-sm font-medium">{camera.id}</span>
      {#if channelLabel}
          ...
          {#if anyDiverged || hasUnsaved}
            {#if anyDiverged}
              <Button
                ...
                size="icon-xs"
                onclick={() => {
                  if (anyPropDiverged) forLinkedAsync(camera, (id) => microscope.profiles.applyProps([id]));
                  if (anyRoiDiverged) forLinkedAsync(camera, (id) => microscope.profiles.applyRoi(id));
                }}
            ...
            <Button
              ...
              onclick={() => {
                forLinkedAsync(camera, (id) => microscope.profiles.saveProps(id));
                forLinkedAsync(camera, (id) => microscope.profiles.saveRoi(id));
              }}
            >
              Save
            </Button>
      ...
      <div class="flex flex-col gap-1">
        <div class="flex items-start justify-between">
          ...
          <SpinBox
            ...
            align="left"
            size="xs"
            onChange={(v) => forLinked(camera, (c) => c.exposure?.patch(v))}
          />
        </div>
        <Slider
        ...
        <div class="flex flex-wrap gap-x-3 gap-y-2 *:min-w-40 *:flex-1">
          ...
          <div class="flex flex-col gap-1">
            ...
            <SpinBox
              ...
              align="left"
              size="xs"
              onChange={(v) => forLinked(camera, (c) => c.frameRate?.patch(v))}
            />
          </div>
        </div>
        <div class="flex flex-wrap gap-x-3 gap-y-2 *:min-w-40 *:flex-1">
          <div class="flex flex-col gap-1">
            ...
            <Select
              ...
              onchange={(v) => forLinked(camera, (c) => c.binning?.patch(Number(v)))}
            />
          </div>
          <div class="flex flex-col gap-1">
            <div class="flex items-center gap-1">
            ...
            <Select
              ...
              onchange={(v) => forLinked(camera, (c) => c.pixelFormat?.patch(v))}
            />
          </div>
        </div>
      </div>
...
{:else}
  <div class={cn('grid grid-cols-[repeat(auto-fit,minmax(20rem,1fr))] items-start gap-4', className)}>
    {#each [...cameras, ...otherCameras] as camera (camera.id)}
      {@render cameraCard(camera, !profileCameraIds.has(camera.id))}
    {/each}
  </div>
{/if}
