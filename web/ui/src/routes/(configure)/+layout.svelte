<script lang="ts">
  ...
  afterNavigate(({ from, to }) => {
    ...
    if (
      toPath === '/' &&
      from &&
      !isInsideConfigure(from.url.pathname) &&
      lastConfigurePath !== '/'
    ) {
  ...
  const cameraIds = $derived(Object.keys(session.cameras));
  const laserIds = $derived(Object.keys(session.lasers));
  const stageIds = $derived([session.stage.x.deviceId, session.stage.y.deviceId, session.stage.z.deviceId]);
  const analogOutIds = $derived(Object.keys(session.analog_outs));

  const groupedIds = $derived(new Set([...cameraIds, ...laserIds, ...stageIds, ...analogOutIds]));
  const otherIds = $derived([...session.devices.devices.keys()].filter((id) => !groupedIds.has(id)));
...
{#snippet deviceRow(id: string)}
  {@const device = session.devices.devices.get(id)}
