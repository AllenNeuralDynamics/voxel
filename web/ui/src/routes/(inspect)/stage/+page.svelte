...
{#if instrument}
  <section class="px-4 pt-2">
    <h2 class="mb-4 text-base font-medium text-fg">Stage</h2>
    {#if axes.length > 0}
          ...
          <div class="rounded-lg border border-border bg-surface/30 p-3">
            <div class="mb-3 flex items-center justify-between gap-2">
              ...
              {#if axis.interface?.type}
                <span class="font-mono text-[0.65rem] text-fg-faint">{axis.interface.type}</span>
              {/if}
            </div>
            <div class="flex flex-col gap-2">
              {#each props as prop (prop.info.name)}
                <div class="grid grid-cols-[8rem_1fr] items-center gap-2">
                  <span class="truncate text-sm text-fg-muted" title={prop.info.desc ?? ''}>{prop.label}</span>
    ...
    {:else}
      <p class="text-sm text-fg-muted">No stage axes mapped.</p>
    {/if}
  </section>
{/if}
