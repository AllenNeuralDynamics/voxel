...
{#if instrument}
  <section class="px-4 pt-2">
    <h2 class="mb-4 text-xl font-medium text-fg">Stage</h2>
    {#if axes.length > 0}
        ...
        {#each axes as { label, axis } (axis.id)}
          ...
          <div class="rounded-lg border border-border bg-surface/30 p-3">
            <div class="mb-3 flex items-center justify-between gap-2">
              <span class="text-lg font-medium text-fg">{label}</span>
              {#if axis.interface?.type}
                <span class="font-mono text-xs text-fg-faint">{axis.interface.type}</span>
              {/if}
            </div>
            <div class="flex flex-col gap-2">
              {#each props as prop (prop.info.name)}
                <div class="grid grid-cols-[9rem_1fr] items-center gap-2">
                  <span class="truncate text-lg text-fg-muted" title={prop.info.desc ?? ''}>{prop.label}</span>
