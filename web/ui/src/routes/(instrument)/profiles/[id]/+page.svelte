...
{#if profile}
	<section class="space-y-4">
		...
		<div>
			<div class="flex items-center gap-1.5">
				...
				<div class="ml-auto flex items-center gap-1.5">
					{#each profile.channels as chId (chId)}
						<span class="bg-element-bg text-fg rounded-full px-1.5 py-px text-[0.65rem]">
			...
			{#if profile.desc}
				<p class="text-fg-muted mt-1 text-xs">{profile.desc}</p>
			{/if}
		</div>
		...
		<Tabs.Root bind:value={activeTab}>
			<Tabs.List class="bg-element-bg text-fg-muted inline-flex h-7 items-center gap-1 rounded-md p-0.5">
				<Tabs.Trigger
					value="waveforms"
					class="hover:text-fg data-[state=active]:bg-canvas data-[state=active]:text-fg inline-flex items-center justify-center rounded-sm px-2.5 py-1 text-[0.65rem] font-medium transition-colors data-[state=active]:shadow-sm"
				>
					Waveforms
				</Tabs.Trigger>
				<Tabs.Trigger
