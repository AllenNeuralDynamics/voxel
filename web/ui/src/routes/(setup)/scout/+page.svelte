<script lang="ts">
  ...
  import { Crosshair, TrashCanOutline, ImageLight, ChevronDown } from '$lib/icons';
      ...
      {#if snaps.focused && previewUrl}
        <div class="flex h-full items-center justify-center overflow-hidden">
          <div class="relative max-h-full max-w-full" style:aspect-ratio={previewAspect}>
            ...
            />
            <div
              class="absolute top-1 left-1 rounded-md border border-border/50 bg-surface/40 text-xs backdrop-blur-md"
            >
              <!-- Snap info + toggle -->
              <button
              ...
              >
                <span class="font-medium text-fg">{snaps.focused.label}</span>
                <span class="shrink-0 font-mono text-fg-muted tabular-nums">
                  {(snaps.focused.stageX / 1000).toFixed(3)}, {(snaps.focused.stageY / 1000).toFixed(3)}, {(
                    snaps.focused.stageZ / 1000
                  ).toFixed(3)} mm
                </span>
                <ChevronDown
                  width="14"
                  height="14"
                  class="ml-auto shrink-0 text-fg-faint transition-transform {showDetails ? 'rotate-180' : ''}"
                />
              </button>
              <!-- Channel details -->
              {#if showDetails}
                    ...
                    {#each channelEntries as [name, ch] (name)}
                      ...
                      {#if hasExposure}
                        <span class="text-fg-muted"
                          >{ch.detection?.exposureTime != null ? `${ch.detection.exposureTime} ms` : '–'}</span
                        >
                      {/if}
                      {#if hasBinning}
                        <span class="text-fg-muted"
                          >{ch.detection?.binning != null ? `${ch.detection.binning}x` : '–'}</span
                        >
                      {/if}
                      {#if hasPower}
                        <span class="text-fg-muted"
                      ...
                      {/if}
                      <span class="text-fg-muted"
                        >{(ch.levelsMin * 100).toFixed(0)}–{(ch.levelsMax * 100).toFixed(0)}%</span
                      >
                    {/each}
                  </div>
                {/if}
