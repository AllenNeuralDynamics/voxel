<script lang="ts">
	...
	import { Select, SortableList } from '$lib/ui/kit';
...
<div class="flex h-full gap-8 overflow-hidden p-4">
	<!-- Left column: session info + plan config + metadata -->
	<div class="bg-panel flex max-w-md min-w-0 flex-1 flex-col overflow-hidden rounded-lg">
		<div class="flex flex-1 flex-col gap-5 overflow-y-auto p-4">
			...
			<section>
				...
				<div class="grid grid-cols-[10rem_1fr] items-center gap-x-3 gap-y-2 text-xs">
		...
		{#if session.info?.session_dir}
			<button
				...
				class="text-fg-muted hover:text-fg shrink-0 cursor-pointer border-t border-border px-4 py-2 text-start text-xs break-all transition-colors"
	...
	<div class="flex min-w-0 flex-1 flex-col overflow-hidden pb-4">
				...
				<div class="flex flex-col gap-5">
					{#each stackGroups as group (group.label)}
						<div class="flex flex-col">
							<!-- Tab header -->
							<div
								class="h-ui-sm border-fg-muted/30 bg-panel flex w-fit items-center rounded-t-lg border-x border-t px-3"
							>
							...
							<div class="border-fg-muted/30 flex flex-col overflow-hidden rounded-tr-lg rounded-b-lg border">
								{#each group.stacks as stack (`${stack.profile_id}:${stack.row},${stack.col}`)}
									<div
										class={cn(
											'h-ui-sm border-fg-muted/30 relative flex items-center gap-3 overflow-hidden border-b px-3 text-xs last:border-b-0',
