...
from voxelstack.asi.v1.models import BoxInfo
from voxelstack.asi.v1.params import ParamCMD, TigerParam
from voxelstack.asi.v1.protocol import ASIProtocol
...
class TigerBox:
    ...
    def _resolve_param_cmd(self, cmd: TigerParam | str | ParamCMD[Any]) -> ParamCMD[Any]:
        if isinstance(cmd, TigerParam):
            return cmd.value
        if isinstance(cmd, ParamCMD):
            return cmd
        if isinstance(cmd, str):
            return ParamCMD(name=cmd, verb=cmd, typ=str)

    @overload
    def get_param[T: int | float | str | bool](self, cmd: ParamCMD[T], axes: Iterable[str]) -> Mapping[str, T]: ...
    @overload
    def get_param(self, cmd: TigerParam | str, axes: Iterable[str]) -> Mapping[str, str]: ...

    def get_param[T: int | float | str | bool](
        self,
        cmd: ParamCMD[T] | TigerParam | str,
        axes: Iterable[str],
    ) -> Mapping[str, T]:
        cmd = self._resolve_param_cmd(cmd)
        axes = [a.upper() for a in axes]
    ...
    @overload
    def set_param[T: int | float | str | bool](self, cmd: ParamCMD[T], mapping: Mapping[str, T]) -> None: ...
    @overload
...
if __name__ == '__main__':
    ...
    SPEED = ParamCMD(name='SPEED', verb='S', typ=float)
    ACCEL = ParamCMD(name='ACCEL', verb='AC', typ=int)
    BACKLASH = ParamCMD(name='BACKLASH', verb='B', typ=float)
