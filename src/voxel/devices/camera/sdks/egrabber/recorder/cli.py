...
class Container(object):
    ...
    def open(self, mode=RECORDER_OPEN_MODE_READ, close_mode=RECORDER_CLOSE_MODE_KEEP, path_suffix=None):
        return self.lib.open_recorder(os.path.join(self.path, path_suffix) if path_suffix else self.path, mode, close_mode=close_mode)
        ...
        if full and self.start_write_time and self.end_write_time:
            ...
            if d:
                ...
                self.bytes_written_per_second_min = \
                    bytes_written_per_second if self.bytes_written_per_second_min is None else \
                    min(self.bytes_written_per_second_min, bytes_written_per_second)
                self.bytes_written_per_second_max = \
                    bytes_written_per_second if self.bytes_written_per_second_max is None else \
                    max(self.bytes_written_per_second_max, bytes_written_per_second)
...
class BenchmarkStats:
    ...
    def push(self, recorderStats):
        if recorderStats.write_duration:
            with self.lock:
                self.megabytes_written_per_second.append(recorderStats.get_bytes_written_per_second(False, False) / 1000000)
                self.megabytes_written_per_second_from_open_to_close.append(recorderStats.get_bytes_written_per_second(True, True) / 1000000)
...
@click.option('--size', default='1GB', metavar='SIZE', show_default=True,
...
@click.option('--warmup',          default=20,        show_default=True, type=click.INT,  help='warmup duration (seconds) excluded from stats')
...
def benchmark(container, size, container_count, buffer_count, buffer_size, warmup, plot, verbose, reuse_existing):
    ...
    try:
        benchmark = threading.Thread(target=thread_fn_benchmark, args=(container_count, buffer_count, buffer_size, warmup))
        benchmark.start()
        if plot:
            ...
            plot_write_rate, = ax_rate.plot([], [], label="Data rate (write only)", color = 'tab:blue', alpha=0.7, linestyle='dotted')
            plot_open_write_close_rate, = ax_rate.plot([], [], label="Data rate (open, write, close)", color = 'tab:blue')
            ...
            plot_perfs_last_minute, = ax_perfs.plot([], [], color='tab:blue', linestyle='dashed', label='Last minute average')
            ...
            def update():
                ...
                if N > 1:
                    ...
                    ax_rate.set_ylim(0, max(max(s.megabytes_written_per_second), max(s.megabytes_written_per_second_from_open_to_close)) * 1.1)
    ...
    if verbose:
        ...
        for rix in range(container_count):
            ...
            fmt = '{:<15}{:>10}{:>15}{:>15}{:>15}'
            na = 'N/A'
            lines = [fmt.format('Container[%d]' % rix, 'count', 'min', 'max', 'avg'),
                     fmt.format('open', s.open_count,
                     ...
                     fmt.format('close', s.close_count,
                     ...
                     fmt.format('write MB/s', s.bytes_written_per_second_count,
                                              na if s.bytes_written_per_second_min is None else '%.3f' % (s.bytes_written_per_second_min / 1000000),
                                              na if s.bytes_written_per_second_max is None else '%.3f' % (s.bytes_written_per_second_max / 1000000),
                                              na if s.bytes_written_per_second_count == 0 else '%.3f' % (s.bytes_written_per_second_sum / s.bytes_written_per_second_count / 1000000)),
                     hline]
            for line in lines:
                click.echo(line)
        click.echo(hline)
    click.echo('Wrote %u bytes in %.6f s (%.3f MB/s)' % (perfs.get_bytes_written(), perfs.get_duration(), perfs.get_MBps()))
