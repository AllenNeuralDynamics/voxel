...
class ConfiguredDevice[T: VoxelDevice]:
    ...
    def _validate_device_setting(self, prop_name: str, setting: ZSetting) -> bool:
        ...
        if not hasattr(self.instance, prop_name):
            self._log.warning(f"Property '{prop_name}' does not exist in device '{self.instance.name}'.")
            return False
        ...
        if not prop_metadata:
            ...
            if prop_descriptor:
                if prop_descriptor.fset is not None:
                    ...
                    self._log.warning(f"Property '{prop_name}' is read-only in device '{self.instance.name}'.")
                    return False
            else:
                self._log.warning(f"Property '{prop_name}' is not defined in device '{self.instance.name}'.")
                return False
        else:
    ...
    def _add_setting_entry(self, prop_name: str, setting: ZSetting, skip_invalid: bool = False) -> None:
        ...
        if not self._validate_device_setting(prop_name, setting):
            if skip_invalid:
                self._log.warning(f"Skipping invalid setting '{prop_name}' for device '{self.instance.name}'.")
                return
            raise ValueError(f"Invalid setting '{prop_name}' for device '{self.instance.name}'.")
