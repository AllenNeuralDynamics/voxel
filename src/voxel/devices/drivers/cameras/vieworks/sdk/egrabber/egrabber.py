...
class EGrabberGenICam(object):
    ...
    def __init__(self, grabber, ix):
        ...
        cE.Eur_EGrabber_CallbackOnDemand_getInterfacePort__from__int(
            grabber._box, c_ix, ct.byref(self.c_interface_port)
        )
    ...
    def run_script(self, script):
        ...
        cE.Eur_EGenTL_genapiRunScript__from__PORT_HANDLE__const_char_p(
            self.grabber.gentl._box, port_handle, utils.to_cstr(script)
        )
...
class EGrabber(object):
    ...
    def __init__(
        self, data, interface=0, device=0, data_stream=0, device_open_flags=DEVICE_ACCESS_CONTROL, remote_required=True
    ):
        ...
        try:
            if is_gentl:
                ...
                cE.Eur_EGrabber_CallbackOnDemand_create__from__Eur_EGenTL__int__int__int__DEVICE_ACCESS_FLAGS__bool8_t(
                    gentl._box,
                    ct.c_int32(interface),
                    ct.c_int32(device),
                    ct.c_int32(data_stream),
                    ct.c_int32(device_open_flags),
                    ct.c_ubyte(remote_required),
                    ct.byref(self._box),
                )
    ...
    def realloc_buffers(self, buffer_count, buffer_size=None):
        ...
        cE.Eur_EGrabber_CallbackOnDemand_reallocBuffers__from__size_t__size_t(
            self._box, ct.c_size_t(buffer_count), ct.c_size_t(buffer_size), ct.byref(bir._box)
        )
        return bir
    ...
    def _get_info(self, port_handle, module, cmd, info_datatype):
        if info_datatype is None:
            ...
            if port_handle:
                ...
                funcname = "Eur_EGrabber_CallbackOnDemand_getInfo__as__InfoCommandInfo__on__{}__from__int32_t".format(
                    module.__name__
                )
        ...
        with utils.Ctype.from_info_datatype(info_datatype) as info:
            if port_handle:
                ...
                funcname = "Eur_EGrabber_CallbackOnDemand_getInfo__as__{}__on__{}__from__int32_t".format(
                    info.ctypename, module.__name__
                )
    ...
    def get_buffer_info(self, buffer_index, cmd, info_datatype=None):
        ...
        if info_datatype is None:
            ...
            info_datatype = dt.value
        with utils.Ctype.from_info_datatype(info_datatype) as info:
            funcname = "Eur_EGrabber_CallbackOnDemand_getBufferInfo__as__{}__from__size_t__BUFFER_INFO_CMD".format(
                info.ctypename
            )
    ...
    def get_buffer_data(self, buffer_index):
        ...
        cE.Eur_EGrabber_CallbackOnDemand_getBufferData__from__size_t(
            self._box, ct.c_size_t(buffer_index), ct.byref(c_nbd)
        )
    ...
    def _gc_read_port(self, port_handle, module, address, size):
        ...
        if port_handle:
            ...
            funcname = "Eur_EGrabber_CallbackOnDemand_gcReadPortData__on__{}__from__uint64_t__void_p__size_t".format(
                module.__name__
            )
    ...
    def _gc_write_port(self, port_handle, module, address, data):
        ...
        if port_handle:
            ...
            funcname = "Eur_EGrabber_CallbackOnDemand_gcWritePortData__on__{}__from__uint64_t__void_p__size_t".format(
                module.__name__
            )
        ...
        with utils.Ctype(ctype) as value:
            if port_handle:
                ...
                getattr(cE, funcname)(
                    ...
                    ct.c_uint64(address),
                    ct.byref(value.box),
                    ct.c_size_t(ct.sizeof(ctype)),
                )
            else:
                funcname = "Eur_EGrabber_CallbackOnDemand_gcReadPortValue__as__{}__on__{}__from__uint64_t".format(
                    value.ctypename, module.__name__
                )
    ...
    def _gc_write_port_value(self, port_handle, module, address, ctype, value):
        if port_handle:
            ...
            getattr(cE, funcname)(
                ...
                port_handle,
                ct.c_uint64(address),
                ct.byref(ctype(value)),
                ct.c_size_t(ct.sizeof(ctype)),
            )
        else:
            funcname = "Eur_EGrabber_CallbackOnDemand_gcWritePortValue__with__{0}__on__{1}__from__uint64_t__{0}".format(
                utils.Ctype.to_attr[ctype][0], module.__name__
            )
    ...
    def _gc_read_port_string(self, port_handle, module, address, size):
        with utils.Ctype.std_string() as s:
            if port_handle:
                ...
                getattr(cE, funcname)(
                    self.gentl._box, port_handle, ct.c_uint64(address), ct.c_size_t(size), ct.byref(s.box)
                )
            else:
                funcname = "Eur_EGrabber_CallbackOnDemand_gcReadPortString__on__{}__from__uint64_t__size_t".format(
                    module.__name__
                )
    ...
    def run_script(self, script):
        ...
        script : str
            ...
            (cf. EGenTL::genapiRunScript).
        """
        cE.Eur_EGrabber_CallbackOnDemand_runScript__from__const_char_p(self._box, utils.to_cstr(script))
    ...
    def interrupt_script(self, script_id):
        ...
        """
        cE.Eur_EGrabber_CallbackOnDemand_interruptScript__from__const_char_p(self._box, utils.to_cstr(script_id))
    ...
    def pop(self, timeout=GENTL_INFINITE):
        ...
        """
        c_nbd = cE.Eur_NewBufferData()
        cE.Eur_EGrabber_CallbackOnDemand_pop__from__uint64_t(self._box, ct.c_uint64(timeout), ct.byref(c_nbd))
        return NewBufferData._from_Eur_EventData(c_nbd)
    ...
    def _get(self, port_handle, module, feature, dtype=None):
        ...
        if dtype is None:
            ...
            if not interfaces:
                raise TypeError("No interface found for feature '{}' in {} module.".format(feature_type, module))
            elif "IInteger" in interfaces:
                dtype = int
            elif "IFloat" in interfaces:
                dtype = float
            # return bool when these queries are used
            elif "IBoolean" in interfaces or feature_type in (
                "@available",
                "@readable",
                "@writeable",
                "@implemented",
        ...
        if dtype in (int, bool):
            ...
            if port_handle:
                ...
                getattr(cE, funcname)(
                    self.gentl._box,
                    port_handle,
                    utils.to_cstr(feature),
                    ct.byref(c_buffer),
                    ct.c_size_t(ct.sizeof(c_buffer)),
                )
            else:
                funcname = funcname.format("Register", module.__name__)
                getattr(cE, funcname)(
                    self._box, utils.to_cstr(feature), ct.byref(c_buffer), ct.c_size_t(ct.sizeof(c_buffer))
                )
            val = c_buffer
        else:
    ...
    def _set(self, port_handle, module, feature, value):
        ...
        if dtype in six.integer_types + (bool,):
            ...
            if port_handle:
                ...
                getattr(cE, funcname)(
                    self.gentl._box, port_handle, utils.to_cstr(feature), ct.byref(value), ct.c_size_t(ct.sizeof(value))
                )
            else:
    ...
    def _attach_event(self, port_handle, module, event_id, event_data=None):
        ...
        if port_handle:
            ...
            funcname = "Eur_EGrabber_CallbackOnDemand_attachEvent__on__{}__from__uint64_t__void_p__size_t".format(
                module.__name__
            )
    ...
    def process_event(self, events="Any", timeout=GENTL_INFINITE):
        ...
        if "Any" in events:
            cE.Eur_EGrabber_CallbackOnDemand_processEvent__with__Any__from__uint64_t(
                self._box, ct.c_uint64(timeout), ct.byref(eventPending)
            )
        else:
            event = ct.c_size_t(EGrabber._events_to_value(events))
            cE.Eur_EGrabber_CallbackOnDemand_processEventFilter__from__size_t__uint64_t(
                self._box, event, ct.c_uint64(timeout), ct.byref(eventPending)
            )
    ...
    def __setattr__(self, name, value):
        if name.startswith("on_") and name.endswith("_event"):
            ...
            if eventType is None:
                raise Exception(
                    "registering callback {} expects event type {} which does not exist".format(name, event)
                )
    ...
    def register_callback(self, event, callback=None, context=None):
        ...
        funcname = "Eur_EGrabber_CallbackOnDemand_set{}Callback__from__Euresys_{}Callback__void_p".format(
...
class GenTLMemory:
    ...
    def __init__(self, buffer_size=0, user_pointer=None):
        ...
        cE.Eur_GenTLMemory_create__from__size_t__void_p(
            ct.c_size_t(buffer_size), ct.c_void_p(user_pointer), ct.byref(self._box)
        )
    ...
    def _announce_and_queue(self, grabber, buffer_count=1):
        bir = BufferIndexRange()
        cE.Eur_EGrabber_CallbackOnDemand_announceAndQueue__from__Eur_GenTLMemory__size_t(
            grabber._box, self._box, ct.c_size_t(buffer_count), ct.byref(bir._box)
        )
        return bir
...
class UserMemory:
    ...
    def __init__(self, buffer, user_pointer=None):
        ...
        cE.Eur_UserMemory_create__from__void_p__size_t__void_p(
            self.base, ct.c_size_t(self.size), ct.c_void_p(user_pointer), ct.byref(self._box)
        )
    ...
    def _announce_and_queue(self, grabber):
        bir = BufferIndexRange()
        cE.Eur_EGrabber_CallbackOnDemand_announceAndQueue__from__Eur_UserMemory(
            grabber._box, self._box, ct.byref(bir._box)
        )
        return bir
...
class BusMemory:
    ...
    def __init__(self, bus_address, size, user_pointer=None):
        ...
        cE.Eur_BusMemory_create__from__uint64_t__size_t__void_p(
            ct.c_uint64(bus_address), ct.c_size_t(size), ct.c_void_p(user_pointer), ct.byref(self._box)
        )
    ...
    def _announce_and_queue(self, grabber):
        bir = BufferIndexRange()
        cE.Eur_EGrabber_CallbackOnDemand_announceAndQueue__from__Eur_BusMemory(
            grabber._box, self._box, ct.byref(bir._box)
        )
        return bir
...
class NvidiaRdmaMemory:
    ...
    def __init__(self, device_address, size, user_pointer=None):
        ...
        cE.Eur_NvidiaRdmaMemory_create__from__void_p__size_t__void_p(
            ct.c_void_p(device_address), ct.c_size_t(size), ct.c_void_p(user_pointer), ct.byref(self._box)
        )
    ...
    def _announce_and_queue(self, grabber):
        bir = BufferIndexRange()
        cE.Eur_EGrabber_CallbackOnDemand_announceAndQueue__from__Eur_NvidiaRdmaMemory(
            grabber._box, self._box, ct.byref(bir._box)
        )
        return bir
...
class UserMemoryArray:
    ...
    def __init__(self, memory, buffer_size):
        ...
        cE.Eur_UserMemoryArray_create__from__Eur_UserMemory__size_t(
            memory._box, ct.c_size_t(buffer_size), ct.byref(self._box)
        )
    ...
    def _announce_and_queue(self, grabber, reverse=False):
        bir = BufferIndexRange()
        cE.Eur_EGrabber_CallbackOnDemand_announceAndQueue__from__Eur_UserMemoryArray__bool8_t(
            grabber._box, self._box, ct.c_ubyte(reverse), ct.byref(bir._box)
        )
        return bir
...
class BufferIndexRange:
    ...
    def _get_data(self):
        ...
        cE.from_box_Eur_BufferIndexRange__from__size_t_p__size_t_p__bool8_t_p(
            self._box, ct.byref(c_begin), ct.byref(c_end), ct.byref(c_reverse)
        )
...
class Buffer:
    ...
    def _ic_in(self):
        if self._box and (self._ic_in_p is None):
            self._ic_in_p = egentl.ImageConvertInput(
                self.get_info(BUFFER_INFO_WIDTH, INFO_DATATYPE_SIZET),
    ...
    def pixels(self):
        ...
        if self._box and (self._pixels is None):
            self._pixels = ct.cast(
                self.get_info(BUFFER_INFO_BASE, INFO_DATATYPE_PTR),
                ct.POINTER(ct.c_ubyte * self.get_info(BUFFER_INFO_DATA_SIZE, INFO_DATATYPE_SIZET)),
            ).contents
        return self._pixels
    ...
    def convert(self, out_fmt, config=IMAGE_CONVERT_OUTPUT_CONFIG_DEFAULT, quality=0, roi=None):
        ...
        ic_out = egentl.ImageConvertOutput(
            self._ic_in.width,
            self._ic_in.height,
            converted_buffer.pixels,
            out_fmt,
            config,
            IMAGE_CONVERT_OUTPUT_OPERATION_COPY,
            quality,
        )
...
class ConvertedBuffer:
    ...
    def _ic_in(self):
        if self._ic_in_p is None:
            self._ic_in_p = egentl.ImageConvertInput(
                self.width, self.height, self.get_address(), self.format, self.get_buffer_size(), self.line_pitch
            )
        return self._ic_in_p
