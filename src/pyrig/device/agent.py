"""Device agent - wraps a device and provides command execution, property access, and streaming."""
...
class DeviceAgent[D: Device]:
    """Wraps a Device and provides command execution, property access, streaming, and publishing.

    Subclass to add device-specific behavior (e.g., CameraAgent for preview).
    """
    ...
    def __init__(self, device: D, stream_interval: float = 0.5):
        ...
        self._thread_pool = ThreadPoolExecutor(max_workers=1, thread_name_prefix=f"Agent-{device.uid}")
        ...
        # Collect agent's own commands and merge
        agent_commands = collect_commands(self)
        self._commands: dict[str, Command] = {**device_commands, **agent_commands}
    ...
    def set_publisher(self, fn: PublishFn) -> None:
        """Set the publish function for this agent."""
        self._publish_fn = fn
