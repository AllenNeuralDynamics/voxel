...
from vxl_cli.check import run_check

from vxl.instrument.bench import InstrumentConfig
from vxlib import load_yaml

TEMPLATE = Path(__file__).parents[2] / "src/vxl/_templates/simulated-local.voxel.yaml"
...
def test_check_invalid_instrument_as_json(tmp_path: Path) -> None:
    ...
    assert payload["targets"][0]["violations"][0]["code"] == "bench.load"
