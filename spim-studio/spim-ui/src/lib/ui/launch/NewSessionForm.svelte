<script lang="ts">
	const {
		...
		rigs: string[];
		onLaunch: (sessionName: string, rigConfig?: string) => void;
	} = $props();
	...
	const isValid = $derived(sessionName.trim().length > 0 && selectedRig);
	...
	async function handleSubmit(e: Event) {
		...
		launching = true;
		try {
			await onLaunch(sessionName.trim(), selectedRig);
		} finally {
			launching = false;
		}
	}
