<script lang="ts">
	...
	import type { Profile, App } from '$lib/app';
	...
	const isDisabled = $derived(app.configLoading || app.isMutating || app.profiles.length === 0);
	const placeholderText = $derived(app.configLoading ? 'Loading profiles...' : 'Select a profile');
	...
	function handleChange(value: string) {
		if (!value) return;
		if (value === app.activeProfileId) return;
		if (app.isMutating) return;

		void app.activateProfile(value);
	}
</script>
...
<div class="flex flex-col gap-1">
	<div class="flex items-center gap-1.5">
		...
		>
			...
			>
				{#if app.profiles.length === 0 && !app.configLoading}
							...
							{#each app.profiles as profile (profile.id)}
								...
								>
									<span class="inline-flex h-4 w-4 items-center justify-center text-xs text-emerald-400">
										{#if app.activeProfileId === profile.id}
	...
	{#if app.error}
		<p class="text-xs text-rose-400">{app.error}</p>
	{/if}
</div>
