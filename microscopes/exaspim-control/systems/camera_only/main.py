...
if __name__ == "__main__":
    from voxel.utils.log import VoxelLogging, get_default_console_handler

    log_filename = f"output_{datetime.now().strftime('%Y-%m-%d_%H-%M-%S')}.log"
    file_handler = FileHandler(log_filename, "w")
    VoxelLogging.setup(level=logging.DEBUG, handlers=[get_default_console_handler(), file_handler])
    logger = VoxelLogging.get_logger("CameraOnlyExaspim")
    # # Setup logging.
