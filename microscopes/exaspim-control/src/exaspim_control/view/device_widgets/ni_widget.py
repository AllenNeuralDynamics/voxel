...
from PySide6.QtWidgets import QComboBox, QSizePolicy, QTreeWidgetItem
from scipy import signal
from exaspim_control.view.base_device_widget import BaseDeviceWidget, create_widget, label_maker, pathGet
from exaspim_control.view.device_widgets.waveform_widget import WaveformWidget
from exaspim_control.view.miscellaneous_widgets.q_non_scrollable_tree_widget import QNonScrollableTreeWidget
from exaspim_control.view.miscellaneous_widgets.q_scrollable_float_slider import QScrollableFloatSlider

if TYPE_CHECKING:
    from exaspim_control.view.miscellaneous_widgets.q_scrollable_line_edit import QScrollableLineEdit
...
class NIWidget(BaseDeviceWidget):
    ...
    def __init__(self, daq: NIDAQ, exposed_branches: dict | None = None, advanced_user: bool = True) -> None:
        """
        ...
        super().__init__(daq, properties={'tasks': daq.tasks}, updating_props=[])
        self.attach_device(daq)
    ...
    def update_waveform(self, channel_name: str) -> None:
        ...
        if item := getattr(self, f'{port_name}.{wl}_plot_item', False):
            ...
            if item is not False:  # Ensure item is not False before passing to remove function
                self.waveform_widget.removeDraggableGraphItem(item)  # type: ignore
        else:
    ...
    def waveform_value_changed(self, value: float, name: str) -> None:
        ...
        dictionary.__setitem__(name_lst[-1], value)  # type: ignore
    ...
    def create_sliders(self, name: str) -> None:
        ...
        slider_color = '#a0b596'
        # active slider color
        hsv_color = QtGui.QColor(slider_color)
        ...
        if 'time' in name:
            ...
            textbox.validator().setRange(0.0, maximum, 0)  # decimals is positional, not keyword
        elif 'volt' in name:
            slider.divisor = 1000
        ...
        if 'amplitude_volts' in name or 'offset_volts' in name:
            ...
            textbox.editingFinished.connect(lambda: slider.setValue(int(float(textbox.text()))))
            ...
            slider.sliderMoved.connect(lambda value: pathGet(self.__dict__, path[0:-1]).__setitem__(path[-1], value))  # type: ignore
    ...
    def create_tree_widget(
        self, name: str, parent: QTreeWidgetItem | None = None, _visited: set[str] | None = None, _depth: int = 0
        ...
        :param _visited: Set of visited paths to prevent infinite recursion
        :type _visited: set[str], optional
        :param _depth: Current recursion depth
        :type _depth: int
        :return: A list of QTreeWidgetItems
        :rtype: list[QTreeWidgetItem]
        """
        # Initialize visited set and check recursion depth
        if _visited is None:
            _visited = set()

        # Prevent infinite recursion
        if _depth > 20 or name in _visited:
            return []
        ...
        except (KeyError, RecursionError, AttributeError):
            # If we can't get the path, return empty list
            return []

        # Safety check: ensure iterable is actually iterable and not None
        if iterable is None:
            return []
        ...
        if hasattr(iterable, 'keys'):
            for i, item in enumerate(iterable):
                ...
                widget = getattr(self, f'{id}_widget', None)
                if widget is not None and widget is not False:
                    tree_item = QTreeWidgetItem(parent, [modified_key])
                    if 'channel' in name:
                        pass
                    elif 'timing' in name:
                        widget = self.remodel_timing_widgets(id, widget)  # type: ignore
                    elif key in ['port', 'waveform']:
                        widget = self.remodel_port_widgets(id, widget)  # type: ignore
                    self.tree.setItemWidget(tree_item, 1, widget)
                else:
                    tree_item = QTreeWidgetItem(parent, [modified_key])
                    children = self.create_tree_widget(f'{name}.{key}', tree_item, _visited.copy(), _depth + 1)
                    tree_item.addChildren(children)
                items.append(tree_item)
                self.check_to_hide(id, tree_item)
            ...
            try:
                for item in iterable:
                    ...
                    if widget is not None and widget is not False:
                        ...
                        tree_item = QTreeWidgetItem(parent, [item])
                        children = self.create_tree_widget(f'{name}.{item}', tree_item, _visited.copy(), _depth + 1)
                        tree_item.addChildren(children)
                    items.append(tree_item)
                    self.check_to_hide(id, tree_item)
            except (TypeError, AttributeError):
                # If iterable is not actually iterable, return what we have
                pass

        return items

    def mappedpathGet(
        self, dictionary: dict, path: list[str], _visited_paths: set[str] | None = None, _depth: int = 0
    ) -> dict:
        """
        ...
        :type path: list[str]
        :param _visited_paths: Set of visited paths to prevent infinite recursion
        :type _visited_paths: set[str], optional
        :param _depth: Current recursion depth
        :type _depth: int
        ...
        # Prevent infinite recursion
        path_str = '.'.join(path)
        if _depth > 20 or path_str in _visited_paths or not path:
            return dictionary

        _visited_paths.add(path_str)

        try:
            dictionary = pathGet(dictionary, path)  # type: ignore
        except KeyError:
            if len(path) >= 2:
                joined_path = '.'.join(path[0:2])
                if joined_path in dictionary:
                    dictionary = self.mappedpathGet(
                        dictionary[joined_path], path[2:], _visited_paths.copy(), _depth + 1
                    )
                else:
                    new_path = [joined_path] + path[2:]
                    dictionary = self.mappedpathGet(dictionary, new_path, _visited_paths.copy(), _depth + 1)
        except (AttributeError, TypeError, RecursionError):
            # If pathGet fails due to invalid structure, return original dictionary
            pass
        finally:
            return dictionary
    ...
    def check_amplitude(self, value: float, name: str) -> None:
        ...
        self.ValueChangedInside.emit(name)
        setattr(self, name, value)
        pathGet(self.__dict__, name_lst[0:-1]).__setitem__(name_lst[-1], value)  # type: ignore
