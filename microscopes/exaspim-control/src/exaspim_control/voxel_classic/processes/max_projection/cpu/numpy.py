...
import numpy as np
import tifffile
from exaspim_control.voxel_classic.processes.max_projection.base import BaseMaxProjection


class CPUMaxProjection(BaseMaxProjection):
    """Voxel driver for the CPU max projection process.
    ...
    def _run(self) -> None:
        ...
        """
        # account for binning
        self._x_projection_count_px = self._x_projection_count_px // self._binning
        self._y_projection_count_px = self._y_projection_count_px // self._binning
        self._z_projection_count_px = self._z_projection_count_px // self._binning
        ...
        if self._x_projection_count_px is None:
            ...
            self.mip_yz = np.zeros(
                (self._frame_count_px, self._row_count_px, len(x_index_list)),
        ...
        if self._y_projection_count_px is None:
            ...
            self.mip_xz = np.zeros(
                (self._frame_count_px, self._column_count_px, len(y_index_list)),
                dtype=self._data_type,
            )
        if self._z_projection_count_px is None:
            ...
            z_projection = True
            if self._z_projection_count_px < 0 or self._z_projection_count_px > self._frame_count_px:
        ...
        while frame_index < self._frame_count_px:
            ...
            if self.new_image.is_set():
                ...
                if z_projection:
                    ...
                    if chunk_index == self._z_projection_count_px - 1 or frame_index == self._frame_count_px - 1:
