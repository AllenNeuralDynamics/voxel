...
class PropertyUpdater:
    ...
    def __init__(
        ...
        self.position_mm = dict.fromkeys(self.tigerbox.ordered_axes, 0.0)
        self.axes_status = dict.fromkeys(self.tigerbox.ordered_axes, False)
        self._thread = threading.Thread(target=self.property_updater)
        self._thread.start()
...
class TigerController(BaseDevice):
    ...
    def __init__(self, uid: str, com_port: str):
        ...
        self.ordered_axes: list[str] = build_config['Motor Axes']
    ...
    def get_current_positions(self) -> dict[str, float]:
        """Get the current positions in millimeters."""
    ...
    def close(self) -> None:
        ...
        self.ser.close() if self.ser else None
    ...
    def set_home(self, *axes: str, wait: bool = True, **kwd_axes: float):
        ...
        return self._set_cmd_args_and_kwds(cmd=Cmds.SETHOME, wait=wait, card_address=None, *args, **kwd_axes)
    ...
    def get_home(self, *axes: str):
        ...
        """
        local_axes = self.ordered_axes or axes  # Default to all axes if none are specified.
        return self._get_axis_value(Cmds.SETHOME, *local_axes)
    ...
    def zero_in_place(self, *axes: str, wait: bool = True):
        ...
        if not axes:  # Default to all lettered axes if none are specified.
            axes = tuple([ax for ax in self.ordered_axes if not ax.isnumeric()])
        for axis in axes:
    ...
    def set_position(self, wait: bool = True, **axes: float):
        ...
        """
        self._set_cmd_args_and_kwds(cmd=Cmds.HERE, wait=wait, card_address=None, **axes)
    ...
    def set_lower_travel_limit(self, *axes: str, wait: bool = True, **kwd_axes: float):
        ...
        """
        axes = tuple(f'{ax}+' for ax in axes)
        # Round axes values in mm to 4 decimal places.
        kwd_axes = {x: round(v, MM_SCALE) for x, v in kwd_axes.items()}
        return self._set_cmd_args_and_kwds(cmd=Cmds.SETLOW, wait=wait, card_address=None, *axes, **kwd_axes)
    ...
    def set_upper_travel_limit(self, *axes: str, wait: bool = True, **kwd_axes: float):
        ...
        # Round axes values in mm to 4 decimal places.
        kwd_axes = {x: round(v, MM_SCALE) for x, v in kwd_axes.items()}
        return self._set_cmd_args_and_kwds(cmd=Cmds.SETUP, wait=wait, card_address=None, *axes, **kwd_axes)
    ...
    def reset_upper_travel_limits(self, *axes: str, wait: bool = True):
        ...
        if not axes:
            axes = tuple(self.ordered_axes)
        return self._reset_setting(Cmds.SETUP, wait=wait, *axes)
    ...
    def set_axis_backlash(self, wait: bool = True, **axes: float):
        ...
        :param wait: wait until the reply has been received.
        """
        self._set_cmd_args_and_kwds(cmd=Cmds.BACKLASH, wait=wait, card_address=None, **axes)
    ...
    def get_position(self, *axes: str):
        ...
        axes = tuple(self._order_axes(list(axes)))
        ...
        if not reply:
            self.log.error('Failed to get position')
            return {}
        axes_positions = [float(v) for v in reply.split()[1:]]
        return dict(zip(axes, axes_positions))
    ...
    def set_speed(self, wait: bool = True, **axes: float):
        ...
        self._set_cmd_args_and_kwds(cmd=Cmds.SPEED, wait=wait, card_address=None, **axes)
    ...
    def set_acceleration(self, **axes: float):
        ...
        self._set_cmd_args_and_kwds(Cmds.ACCEL, card_address=None, wait=True, **axes)
    ...
    def bind_axis_to_joystick_input(self, **axes: JoystickInput):
        ...
        .. code-block:: python
            ...
            box.bind_axis_to_joystick(x=JoystickInput.Y, y=JoystickInput.CONTROL_KNOB)
    ...
    def set_joystick_axis_polarity(self, **axes: JoystickPolarity):
        ...
        .. code-block:: python
            ...
            box.set_joystick_polarity(x=JoystickPolarity.DEFAULT, y=JoystickPolarity.INVERTED)
    ...
    def get_encoder_ticks_per_mm(self, axis: str):
        ...
        reply = self.send(cmd_str)
        if reply is None:
            self.log.error('Failed to get encoder ticks per mm')
            return None
    ...
    def get_axis_id(self, axis: str):
        ...
        reply = self.send(cmd_str)
        if reply is None:
            self.log.error('Failed to get axis id')
            return None
    ...
    def set_axis_control_mode(
        self,
        wait: bool = True,
        **axes: MicroMirrorControlMode | PiezoControlMode | TunableLensControlMode | int | str,
    ):
        ...
        self._set_cmd_args_and_kwds(cmd=Cmds.PM, wait=wait, card_address=None, **kwargs)
    ...
    def scanr(
        ...
        scan_stop_mm: float | None = None,
        num_pixels: int | None = None,
        retrace_speed_percent: float = DEFAULT_SPEED_PERCENT,
        wait: bool = True,
    ):
    ...
    def setup_array_scan(
        ...
        delta_y_mm: float = 0,
        theta_deg: float = 0,
        x_start_mm: int | None = None,
        y_start_mm: int | None = None,
        pattern: ScanPattern = ScanPattern.RASTER,
        card_address: int | None = None,
        wait: bool = True,
    ):
        ...
        if card_address is None:
            ...
            if len(cards) != 1:
                raise RuntimeError('Cannot infer the card address. It must bespecified explicitly.')
    ...
    def start_array_scan(self, wait: bool = True):
        ...
        if self._array_scan_card_addr is None:
            raise RuntimeError(
                'Cannot infer the card address for which to apply the sttings. setup_array_scan must be run first.'
            )
        card_address = self._array_scan_card_addr
    ...
    @thread_locked
    def reset_ring_buffer(self, axis: str | None = None, wait: bool = True):
    ...
    def queue_buffered_move(
        ...
        **axes: float,
        ...
        :param wait: wait until the reply has been received.
        """
        self._set_cmd_args_and_kwds(cmd=Cmds.LOAD, card_address=None, wait=wait, **axes)
    ...
    def set_ttl_pin_modes(
        ...
        .. code-block:: python
            ...
            box.set_ttl_pin_modes(
                In0Mode.REPEAT_LAST_REL_MOVE, Out0Mode.PULSE_AFTER_MOVING, reverse_output_polarity=True
            )

            # OR: make the input ttl pin trigger a predefined ARRAY movement.
            box.set_ttl_pin_modes(
                IN0Mode.ARRAY_MODE_MOVE_TO_NEXT_POSITION,
        ...
        param_str = f'{in0_str}{out0_str}{auxstate_str}{polarity_str}{auxmask_str}{auxmode_str}'.rstrip()
        ...
        # be changed.
            ...
            if len(cards) != 1:
                raise RuntimeError(
                    'Cannot infer the card address of the X and Y axes. card_address must be explicitly specified.'
                )
    ...
    def get_ttl_output_state(self, wait: bool = True):
        ...
        if not reply:
            self.log.error('Failed to get TTL output state.')
            return None
        return bool(int(reply.lstrip(':A ')))

    def is_moving(self):
        """True if any axes is moving. False otherwise. Blocks."""
        return bool(any(self.property_updater.axes_status))
    ...
    def are_axes_moving(self, *axes: str):
        ...
        if sleep_time > 0:
            sleep(sleep_time)
        if not axes:  # Default to all lettered axes if none are specified.
            axes = tuple(x for x in self.ordered_axes if not x.isnumeric())
        axes_str = ''.join([f' {x.upper()}?' for x in axes])
        # Send the inquiry. Handle: ":A \r\n" and ":A\r\n" and remove ":A " from reply
        reply = self.send(f'{Cmds.RDSTAT.value + axes_str}\r')
        if not reply:
            self.log.error('Failed to get axis states.')
            return {}
        ...
        if reply == ACK:
            ...
            reply = self.send(f'{Cmds.RDSTAT.value + axes_str}\r')
            if reply is None:
                self.log.error('Failed to get axis states after re-issuing command.')
                return {}
            reply = reply.rstrip().rstrip('\r\n').lstrip(ACK).lstrip()

        axis_states = list(reply)
        if 'B' not in reply and 'N' not in reply:
            raise RuntimeError(f"Error. Cannot tell if axes are moving. Received: '{reply}'")
    ...
    def wait_on_axis(self, *axes: str):
        ...
        while any(self.is_axis_moving(axis) for axis in axes):
            sleep(0.01)
    ...
    def send(self, cmd_str: str, read_until: str = '\r\n', wait: bool = True):
        ...
        self.log.debug(f'Sending: {cmd_str!r}')
        # self.ser.reset_input_buffer()
        # self.ser.reset_output_buffer()
        if not self.ser:
        ...
        while True:
            ...
            except SyntaxError:  # Technically, this could be a skipped reply.
                self.log.error(f'Error occurred when sending: {cmd_str!r}')
                raise
            break
        return reply
    ...
    def get_info(self, axis: str):
        ...
        reply = self.send(cmd_str)
        if not reply:
            self.log.error(f'Failed to get info for axis {axis}')
            return None
        reply = reply.strip('\r\n')
    ...
    def _get_axis_to_card_mapping(build_config: dict):
        ...
        """
        axis_to_card = {}
        curr_card_index = dict.fromkeys(set(build_config['Hex Addr']), 0)
    ...
    def _get_axis_to_type_mapping(build_config: dict):
        ...
        """
        axis_to_type = {}
        # curr_card_index = dict.fromkeys(set(build_config["Axis Types"]), 0)
    ...
    def _order_axes(self, axes: list[str]) -> list[str]:
    ...
    def _get_axis_value(self, cmd: Cmds, *axes: str):
        ...
        reply = self.send(cmd_str)
        if not reply:
            self.log.error(f'Failed to get info for axis {axes}')
            return None
        reply = reply.rstrip('\r\n').strip(ACK).split()
    ...
    def _get_card_modules(self, card_address):
        ...
        if not reply:
            self.log.error(f'Failed to get modules for card {card_address}')
            return None
        for line in reply.split('\r'):
    ...
    def check_reply_for_errors(reply: str):
        ...
        try:
            ...
            raise SyntaxError(f'Error. TigerController replied with error code: {error_enum!s}.')
        except ValueError:
            pass
