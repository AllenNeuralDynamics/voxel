"""SPIM Qt application state manager.

This module provides the central application state for SPIM Qt, managing:
- System configuration (from ~/.spim/system.yaml)
...
# Reuse system config from spim-studio (shares ~/.spim/ directory)
from spim_studio.system import (
    SessionDirectory,
    SessionRoot,
    SystemConfig,
    get_rig_path,
...
class SpimQtApp(QObject):
    ...
    def _load_system_config(self) -> None:
        """Load system configuration from ~/.spim/system.yaml."""
        try:
    ...
    def available_rigs(self) -> list[str]:
        """Available rig configurations from ~/.spim/rigs/."""
        return list_rigs()
