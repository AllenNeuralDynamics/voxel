...
class RigService:
    ...
    async def get_status(self) -> RigStatus:
        ...
        acq_task_active = self.rig._frame_task is not None and self.rig._frame_task._is_setup
    ...
    async def _broadcast_waveforms(self):
        ...
        if self.rig._frame_task:
            try:
                waveforms = self.rig._frame_task.get_written_waveforms(target_points=1000)
    ...
    async def _handle_waveform_request(self, client_id: str):
        ...
        # if self.rig._frame_task:
        #     try:
        #         waveforms = self.rig._frame_task.get_written_waveforms()
    ...
    async def _subscribe_to_device_streams(self):
        ...
        for device_id, handle in self.rig.handles.items():
            ...
            def make_forwarder(dev_id: str):
                async def forwarder(topic: str, props: PropsResponse):
                    try:
                        # PropsResponse is already parsed
                        ...
                        await self._broadcast(f"device/{dev_id}/{topic_suffix}", payload)
                    except Exception as e:
            ...
            await handle.subscribe("properties", make_forwarder(device_id))
...
async def list_devices(rig: SpimRig = Depends(get_rig)) -> dict:
    ...
    for device_id, handle in rig.handles.items():
        try:
            interface = await handle.get_interface()
