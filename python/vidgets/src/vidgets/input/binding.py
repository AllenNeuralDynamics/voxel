...
class FieldBinder[ReadT, WriteT](QObject):
    ...
    def __init__(
        ...
        *,
        ...
        debounce_ms: int = 150,
        settle_ms: int = 300,
        equals: Callable[[ReadT | WriteT, ReadT | WriteT], bool] = lambda a, b: a == b,
        ...
        self._settle_s = max(0, settle_ms) / 1000.0
    ...
    def _flush(self) -> None:
        ...
        self._suppress_until = time.monotonic() + self._settle_s

    def update(self, new_value: ReadT) -> None:
        # ignore stale state briefly after writing
        if time.monotonic() < self._suppress_until:
            return
        if self._current is None or not self._equals(self._current, new_value):
