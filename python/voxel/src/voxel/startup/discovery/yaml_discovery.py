...
class YAMLRepository[T: ChannelDefinition | ProfileDefinition](Repository[T]):
    def __init__(self, root_path: Path, model: type[T]):
        ...
        if not yaml_files:
            raise FileNotFoundError(f"Directory must contain at least one YAML file: {self._root}")
        self._model = model
    ...
    def delete(self, uid: str) -> None:
        ...
        if path.exists():
            ...
            raise FileNotFoundError(f"No such file to delete: {path}")
...
class YAMLInstrumentLoader(InstrumentConfigLoader):
    ...
    def validate_path(path: Path) -> None:
        ...
        missing = []
        if not path.is_dir():
            raise FileNotFoundError(f"Configuration directory not found at: {path.resolve()}")
...
class YAMLInstrumentDiscovery(InstrumentDiscovery):
    ...
    def __init__(self, instruments_root_dir: str = ".voxel/instruments") -> None:
        ...
        if not self.root_path.is_dir():
            raise FileNotFoundError(f"Instruments root directory not found at: '{self.root_path.resolve()}'")
