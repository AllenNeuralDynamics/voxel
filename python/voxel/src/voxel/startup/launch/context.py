...
from .step import BasicLaunchStepResult, LaunchReportEntry, LaunchStep, LaunchStepResult

if TYPE_CHECKING:
    from voxel.instrument import Instrument, InstrumentNode
...
class LaunchContext:
    ...
    def latest(self) -> LaunchStepResult[Any]:
        ...
        return current or BasicLaunchStepResult(step=LaunchStep.NEW, data=None)
    ...
    def advance(self, result: LaunchStepResult[Any]) -> None:
        ...
        if result.step not in self._results:
            msg = f'No context slot for step {result.step}'
            raise ValueError(msg)
        # 2) guard ordering
        if not self.can_advance(result.step):
            msg = f'Cannot jump from {self._current_step.name} to {result.step.name}'
            raise ValueError(msg)
