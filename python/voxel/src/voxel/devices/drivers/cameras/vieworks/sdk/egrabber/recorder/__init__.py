...
class ChapterInfo:
    ...
    def __init__(self, recorder):
        ...
        self.base_record_index = recorder.get(RECORDER_PARAMETER_RECORD_INDEX) - recorder.get(
...
class Recorder:
    ...
    def export(self, path, count, export_pixel_format=None, on_progress=None):
        ...
        return self.recorder_lib._export(
            self.handle, path, count, export_pixel_format, progress_callback, ctypes.py_object(self)
        )
    ...
    def find_chapter_by_record_index(self, record_index):
        for chapter in self.chapters:
            if (
                chapter
                and chapter.base_record_index <= record_index
                and record_index < chapter.base_record_index + chapter.record_count
            ):
                return chapter
...
class RecorderLibrary:
    ...
    def _open(self, path, mode):
        ...
        RecorderError.check(
            self._dll.RecorderOpen(
                to_cstr(path), _RECORDER_OPEN_MODE(mode), ctypes.byref(c_handle), ctypes.byref(c_file_version)
            )
        )
    ...
    def _set_parameter_string(self, handle, parameter, value):
        RecorderError.check(
            self._dll.RecorderSetParameterString(
                _RECORDER_HANDLE(handle), _RECORDER_PARAMETER(parameter), to_cstr(value)
            )
        )
    ...
    def _get_parameter_string(self, handle, parameter):
        ...
        RecorderError.check(
            self._dll.RecorderGetParameterString(
                _RECORDER_HANDLE(handle), _RECORDER_PARAMETER(parameter), None, ctypes.byref(c_size)
            )
        )
        c_value = (ctypes.c_char * c_size.value)()
        RecorderError.check(
            self._dll.RecorderGetParameterString(
                _RECORDER_HANDLE(handle), _RECORDER_PARAMETER(parameter), ctypes.byref(c_value), ctypes.byref(c_size)
            )
        )
    ...
    def _set_parameter_integer(self, handle, parameter, value):
        RecorderError.check(
            self._dll.RecorderSetParameterInteger(
                _RECORDER_HANDLE(handle), _RECORDER_PARAMETER(parameter), ctypes.c_int64(value)
            )
        )
    ...
    def _get_parameter_integer(self, handle, parameter):
        ...
        RecorderError.check(
            self._dll.RecorderGetParameterInteger(
                _RECORDER_HANDLE(handle), _RECORDER_PARAMETER(parameter), ctypes.byref(c_value)
            )
        )
        ...
        if info and buffer:
            ...
            def check_len():
                if info.size > len(buffer):
                    raise BufferTooSmall(
                        "buffer length({}) must be greater than or equal to info.size({})".format(
            ...
            if type(buffer).__name__.startswith("c_char_Array_"):
                ...
                raise InvalidParameterType(
                    "buffer must be ctypes.c_char_Array_, bytearray or bytes for write operation"
                )
            RecorderError.check(
                self._dll.RecorderWrite(
                    _RECORDER_HANDLE(handle), ctypes.byref(info), buffer_ptr or ctypes.byref(c_buffer), None, None, None
                )
            )
            return
        else:
    ...
    def _read(self, handle):
        ...
        RecorderError.check(
            self._dll.RecorderRead(_RECORDER_HANDLE(handle), None, None, ctypes.byref(c_buffer_size), None)
        )
        info = RECORDER_BUFFER_INFO()
        c_buffer = (ctypes.c_char * (c_buffer_size.value if c_buffer_size.value else 1))()
        RecorderError.check(
            self._dll.RecorderRead(
                _RECORDER_HANDLE(handle), ctypes.byref(info), ctypes.byref(c_buffer), ctypes.byref(c_buffer_size), None
            )
        )
    ...
    def _read_info(self, handle):
        ...
        RecorderError.check(
            self._dll.RecorderRead(
                _RECORDER_HANDLE(handle), ctypes.byref(info), None, ctypes.byref(c_buffer_size), None
            )
        )
        return info
    ...
    def _export(self, handle, path, count, export_pixel_format, progress_callback, progress_context):
        ...
        RecorderError.check(
            self._dll.RecorderExport(
                _RECORDER_HANDLE(handle),
                to_cstr(path),
                ctypes.byref(c_count),
                ctypes.c_uint32(export_pixel_format),
                ctypes.c_uint32(0),
                progress_callback,
                progress_context,
            )
    ...
    def _set(self, parameter, value):
        ...
        if dtype in six.integer_types:
            RecorderError.check(
                self._dll.RecorderSetParameterInteger(None, _RECORDER_PARAMETER(parameter), ctypes.c_int64(value))
            )
        elif dtype in (str, six.text_type):
            RecorderError.check(
                self._dll.RecorderSetParameterString(None, _RECORDER_PARAMETER(parameter), to_cstr(value))
            )
        else:
            raise InvalidParameterType("value must be {} or {}".format(six.integer_types, six.text_type))
