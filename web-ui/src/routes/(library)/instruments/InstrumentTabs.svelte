  ...
  const tabs = $derived<{ id: InstrumentTab; label: string }[]>([
    ...(hal || instrumentState
      ? [
          ...
          { id: 'state' as const, label: instrumentState?.kind === 'default' ? 'Default State' : 'Current State' },
          ...
          <section>
            <div class="mb-2 flex items-center gap-3">
              <h2 class="text-base font-medium tracking-wide text-fg-muted uppercase">
                {instrumentState.kind === 'default' ? 'Default State' : 'Current State'}
              </h2>
              {#if instrumentState.kind === 'state' && instrumentState.activeDefaults}
