<script lang="ts">
  ...
  import { getVoxelStation, type TaskPatch, type TileOrder } from '$lib/model';
      ...
      <button
        ...
        class={cn(
          ...
          selected
            ...
            : 'border-control-line bg-element-bg hover:border-fg-muted'
        )}
      >
    ...
    <div data-cell-edit class="cell justify-end">
      <Popover.Root>
        ...
        <Popover.Portal>
          <Popover.Content
            align="end"
            class="z-50 min-w-40 rounded border border-line-muted bg-floating p-1 text-fg shadow-md"
          >
              ...
              >
                <span
                  class={cn(
                    ...
                    active ? 'border-primary bg-primary text-primary-fg' : 'border-control-line bg-element-bg'
                  )}
                >
...
{#snippet nudgeButton(Icon: Component, label: string, onNudge: () => void)}
  <button
    class="flex h-ui-xs w-5 items-center justify-center rounded border border-control-line
...
<div class="flex h-full min-h-0 min-w-0 flex-col">
  ...
  <div class="flex min-h-0 flex-1 flex-col overflow-hidden">
    <div class="flex flex-wrap items-center gap-3 border-0 border-line-muted px-3 pb-2">
    ...
    <div
      ...
      class="grid flex-1 auto-rows-min grid-cols-[auto_auto_auto_auto_auto_auto_1fr] content-start overflow-y-auto px-3"
    >
      {#if rows.length === 0}
        <div class="col-span-full flex min-h-32 items-center justify-center p-4">
          <p class="text-lg text-fg-faint">No tasks — add tasks from the Grid page</p>
        </div>
      {:else}
        <!-- Column header -->
        <div class="col-span-full grid grid-cols-subgrid text-lg font-medium text-fg-muted">
          ...
          <span class="cell cell-head justify-between">{@render headerMatch('x')}X ({unit.label})</span>
          <span class="cell cell-head justify-between">{@render headerMatch('y')}Y ({unit.label})</span>
          <span class="cell cell-head justify-between">{@render headerMatch('start')}Z Start ({unit.label})</span>
          <span class="cell cell-head justify-between">{@render headerMatch('end')}Z End ({unit.label})</span>
    ...
    <Dialog.Content>
      ...
      <Dialog.Footer>
        <button
          ...
          class="rounded border border-control-line px-3 py-1.5 text-lg text-fg-muted transition-colors hover:bg-element-hover hover:text-fg"
        >
          Cancel
        </button>
        <button
...
<style>
  ...
  .cell {
    --cell-border: 1px solid var(--color-line-faint);
    display: flex;
    align-items: center;
