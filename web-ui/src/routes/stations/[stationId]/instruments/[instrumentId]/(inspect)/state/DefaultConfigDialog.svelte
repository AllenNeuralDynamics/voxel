...
<script lang="ts">
  ...
  let { mode = $bindable(null) }: { mode: DefaultDialogMode | null } = $props();
  ...
  function confirm(): void {
    if (!instrument || mode === null) return;
...
<Dialog.Root open={mode !== null} onOpenChange={(open) => !open && (mode = null)}>
  <Dialog.Content size="xxl" showCloseButton={false}>
    <Dialog.Header>
      <Dialog.Title>{title}</Dialog.Title>
    </Dialog.Header>
    <p class="text-lg text-fg-muted">
      {restoring ? 'Overwrite the current state with the saved default.' : 'Save the current state as the new default.'}
    </p>
    {#if instrument}
    ...
    {/if}
    <Dialog.Footer>
      <Button variant="ghost" onclick={() => (mode = null)}>Cancel</Button>
