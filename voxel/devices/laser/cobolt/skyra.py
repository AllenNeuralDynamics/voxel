...
class SkyraLaser(BaseLaser):
            ...
            return int(
                round(
                    self._coefficients_curve().subs(
                        symbols("x"), self._current_setpoint
                    )
                )
            )
        else:
    ...
    def power_setpoint_mw(self, value: float or int):
        if self.modulation_mode != "off":
            ...
            for sol in solutions:
                ...
                if round(sol) in range(
                    int(self._min_current_ma), int(self._max_current_ma)
                ):
                    # setpoint must be integer
                    self._current_setpoint = int(round(sol))
                    # set lasser current setpoint to ma value
                    self._inst.send_cmd(
                        f"{self._prefix}Cmd.CurrentSetpoint" f"{self._current_setpoint}"
                    )
                    return
    ...
    def modulation_mode(self):
        ...
        if self._inst.send_cmd(f"{self._prefix}Query.ModulationMode") == BoolVal.OFF:
            ...
            self._inst.send_cmd(f"{self._prefix}Query.AnalogModulationMode")
            == BoolVal.ON
        ):
            return "analog"
        else:
