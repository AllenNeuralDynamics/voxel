/**
 * Stage: unified controller handling stage positioning, grid calculations, and FOV visualization.
 */

import type { RigManager, StageConfig } from '$lib/core';
 ...
 */
export class Axis {
	readonly #manager: RigManager;
	readonly #deviceId: string;

	constructor(manager: RigManager, deviceId: string) {
	...
	isConnected = $derived.by(() => {
		return this.#manager.devices.getDevice(this.#deviceId)?.connected ?? false;
	});

	// Derived state properties
	position = $derived.by(() => {
		const val = this.#manager.devices.getPropertyValue(this.#deviceId, 'position_mm');
		return typeof val === 'number' ? val : 0;
	});

	lowerLimit = $derived.by(() => {
		const val = this.#manager.devices.getPropertyValue(this.#deviceId, 'lower_limit_mm');
		return typeof val === 'number' ? val : 0;
	});

	upperLimit = $derived.by(() => {
		const val = this.#manager.devices.getPropertyValue(this.#deviceId, 'upper_limit_mm');
		return typeof val === 'number' ? val : 100;
	});

	isMoving = $derived.by(() => {
		const val = this.#manager.devices.getPropertyValue(this.#deviceId, 'is_moving');
		return typeof val === 'boolean' ? val : false;
	});

	// Movement methods
	move(position: number): void {
		this.#manager.devices.executeCommand(this.#deviceId, 'move_abs', [position], { wait: false });
	}

	async halt(): Promise<void> {
		await this.#manager.devices.executeCommand(this.#deviceId, 'halt');
	}
}
...
export class Stage {
	...
	constructor(manager: RigManager, previewer: Previewer) {
		this.#manager = manager;
		this.#previewer = previewer;

		// Get stage configuration
		this.config = manager.config?.stage;
	...
	#getFOVConfig() {
		return {
			...
			w: this.#manager.activeProfile?.fovDimensions?.width ?? 5,
			h: this.#manager.activeProfile?.fovDimensions?.height ?? 5
		};
	}
