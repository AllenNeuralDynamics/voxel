...
class SessionConfig(BaseModel):
    ...
    This model represents the complete session file (.spim.yaml) with:
    - rig: The full SpimRigConfig
    ...
    def from_yaml(cls, path: Path) -> "SessionConfig":
        """Load from .spim.yaml file, preserving anchors for round-tripping."""
        with open(path) as f:
            raw_data = yaml.load(f)

        config = cls(
            rig=SpimRigConfig.model_validate(raw_data.get("rig", {})),
    ...
    def create_new(cls, rig_config_path: Path) -> "SessionConfig":
        ...
        rig = SpimRigConfig.model_validate(rig_data)
...
class Session:
    """Manages a SPIM acquisition session with state persistence.

    Sessions are stored as a single .spim.yaml file with:
    ...
    async def launch(cls, session_dir: Path, config_path: Path | None = None) -> "Session":
        ...
        Args:
            ...
            config_path: Path to rig config (only used for new sessions).
                If session_dir already has a session.spim.yaml, this is ignored.
        """
    ...
    @property
    def rig(self) -> SpimRig:
        """Get the underlying SpimRig instance."""
        return self._rig
