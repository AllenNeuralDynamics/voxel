"""Frame acquisition task using DAQ handle."""
...
class FrameTaskData(BaseModel):
    """Frame task timing and waveform data (without port assignments)."""
...
class FrameTaskConfig(FrameTaskData):
    """Frame task configuration with port assignments."""

    ports: dict[str, str]

    @classmethod
    def from_data(cls, data: FrameTaskData, ports: dict[str, str]) -> "FrameTaskConfig":
...
class DAQFrameTask:
    """Orchestrates DAQ frame acquisition."""
